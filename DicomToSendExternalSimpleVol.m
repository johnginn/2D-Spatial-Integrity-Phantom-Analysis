function [] = DicomToSendExternalSimpleVol( dicomPath, outputDirectory, outputFilename )
%%
...
for ifile = 1:1:nit-2
    ...
    header = BuildSendExternalHeader( info, 1 );
...
header1 = sprintf( '%s\n%s%d\n%s%d\n%s%d\n%s%d\n%s%d\n',...
    s1,s2,anatomicalSliceNumber,s3,anatomicalSliceNumber,s4,sliceNumber,...
    s5,info.Columns,...
    s6,info.Rows );
%header1 = sprintf( '%s\n%s%d\n%s%d\n%s%d\n%s%d\n%s%d\n',...
